const orderTeamsByCurrentUser = (battle, user, trainerDetail) => {
  let currentUserTeam = null;
  let otherUserTeam = null;
  if (battle.teams.length === 1) {
    currentUserTeam =
      trainerDetail[battle.teams[0].trainer].email === user.email ? battle.teams[0] : null;
    otherUserTeam = currentUserTeam === null ? battle.teams[0] : null;
  } else if (battle.teams.length === 2) {
    currentUserTeam =
      trainerDetail[battle.teams[0].trainer].email === user.email
        ? battle.teams[0]
        : battle.teams[1];
